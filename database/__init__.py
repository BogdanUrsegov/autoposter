...
_USERBOT_COLUMNS = {
    ...
    "gift_media": "ALTER TABLE userbot_accounts ADD COLUMN gift_media TEXT DEFAULT '[]'",
    "followup2_text": "ALTER TABLE userbot_accounts ADD COLUMN followup2_text TEXT DEFAULT ''",
    "followup2_parse_mode": "ALTER TABLE userbot_accounts ADD COLUMN followup2_parse_mode VARCHAR(16) DEFAULT 'HTML'",
    "followup2_entities": "ALTER TABLE userbot_accounts ADD COLUMN followup2_entities TEXT DEFAULT '[]'",
    "followup3_text": "ALTER TABLE userbot_accounts ADD COLUMN followup3_text TEXT DEFAULT ''",
    "followup3_parse_mode": "ALTER TABLE userbot_accounts ADD COLUMN followup3_parse_mode VARCHAR(16) DEFAULT 'HTML'",
    "followup3_entities": "ALTER TABLE userbot_accounts ADD COLUMN followup3_entities TEXT DEFAULT '[]'",
    "max_greets_hour": "ALTER TABLE userbot_accounts ADD COLUMN max_greets_hour INTEGER DEFAULT 1000",
    "nudge_30_text": "ALTER TABLE userbot_accounts ADD COLUMN nudge_30_text TEXT DEFAULT 'ну что там???'",
    "nudge_6h_text": "ALTER TABLE userbot_accounts ADD COLUMN nudge_6h_text TEXT DEFAULT 'эййййй, скоро кончатся призы!!!'",
    "nudge_24h_text": "ALTER TABLE userbot_accounts ADD COLUMN nudge_24h_text TEXT DEFAULT 'ну что там???'",
}

_USERBOT_LEAD_COLUMNS = {
    "hello_replied": "ALTER TABLE userbot_leads ADD COLUMN hello_replied BOOLEAN DEFAULT FALSE",
    "gift_replied": "ALTER TABLE userbot_leads ADD COLUMN gift_replied BOOLEAN DEFAULT FALSE",
    "followup2_replied": "ALTER TABLE userbot_leads ADD COLUMN followup2_replied BOOLEAN DEFAULT FALSE",
    "followup3_replied": "ALTER TABLE userbot_leads ADD COLUMN followup3_replied BOOLEAN DEFAULT FALSE",
}
...
def _migrate_sync(connection) -> None:
    ...
    if "userbot_accounts" in tables:
        ...
        for name, sql in _USERBOT_COLUMNS.items():
            if name not in cols:
                connection.execute(text(sql))
    if "userbot_leads" in tables:
        cols = {c["name"] for c in insp.get_columns("userbot_leads")}
        for name, sql in _USERBOT_LEAD_COLUMNS.items():
            if name not in cols:
                connection.execute(text(sql))
