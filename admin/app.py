...
async def list_userbots(request: Request, _: None = Depends(require_admin)):
    ...
    from bot.utils.envfile import api_configured
...
async def update_userbot(aid: int, request: Request, _: None = Depends(require_admin)):
    ...
    if "max_greets_hour" in body:
        async with SessionLocal() as session:
            acc = await session.get(UserBotAccount, aid)
            if acc:
                try:
                    acc.max_greets_hour = max(1, min(5000, int(body.get("max_greets_hour") or 1000)))
