  ...
  root.innerHTML = `
    ...
    <div class="panel">
      ...
      <div class="grid2">
        ...
        <div class="field"><label>Макс. ответ, сек</label><input id="ub-sla" value="${esc(g.max_reply_sec)}"></div>
    ...
    ${items
      .map((a) => {
        ...
        return `<div class="panel">
          ...
          <div class="field"><label>Подарок (HTML / &lt;code&gt; = копирование)</label><textarea id="ub-g-${a.id}">${esc(a.gift_text || "")}</textarea></div>
          <div class="field">
            <label>Скрины к подарку (${(a.gift_media || []).length})</label>
            <div class="muted" style="margin-bottom:6px">${(a.gift_media || []).map((p) => esc(String(p).split("/").pop())).join(", ") || "нет"}</div>
            <input id="ub-media-${a.id}" type="file" accept="image/*" multiple>
            <button class="btn small ghost" type="button" onclick="ubClearMedia(${a.id})">Очистить скрины</button>
          </div>
          <div class="field"><label>Текст 2 — после ответа на подарок</label><textarea id="ub-f2-${a.id}">${esc(a.followup2_text || "")}</textarea></div>
          <div class="field"><label>Текст 3 — после ответа на текст 2</label><textarea id="ub-f3-${a.id}">${esc(a.followup3_text || "")}</textarea></div>
          <div class="grid2">
...
window.saveUbGlobals = async function () {
  const body = {
    max_reply_sec: Number($("#ub-sla").value || 45),
    min_send_gap: Number($("#ub-gap").value || 0.6),
  };
...
window.ubClearMedia = async function (id) {
  await api("/api/userbots/" + id, {
    method: "PUT",
    body: JSON.stringify({ gift_media: [] }),
  });
  render();
};

window.saveUb = async function (id) {
  const media = [...(cache.userbots[id]?.gift_media || [])];
  const input = document.getElementById("ub-media-" + id);
  ...
  await api("/api/userbots/" + id, {
    ...
    body: JSON.stringify({
      ...
      followup2_text: $("#ub-f2-" + id).value,
      followup3_text: $("#ub-f3-" + id).value,
      ...
      followup2_parse_mode: "HTML",
      followup3_parse_mode: "HTML",
      gift_media: media.slice(0, 10),
    }),
  });
  alert("Сохранено");
  render();
