...
async def _ub_card(account_id: int) -> tuple[str, InlineKeyboardMarkup] | None:
    async with SessionLocal() as session:
        ...
        try:
            media_n = len(json.loads(getattr(row, "gift_media", None) or "[]"))
        except Exception:
            media_n = 0
        f2 = _short(getattr(row, "followup2_text", None) or "")
        f3 = _short(getattr(row, "followup3_text", None) or "")
        text = (
            ...
            f"Привет: {hello}\n\n"
            f"<b>Подарок:</b> {gift_preview}\n"
            f"🖼 Скринов: <b>{media_n}</b>\n"
            f"<b>Текст 2 (после ответа на подарок):</b> {f2}\n"
            f"<b>Текст 3 (после ответа на 2):</b> {f3}\n\n"
        ...
        kb = [
            ...
            [
                InlineKeyboardButton(text="🖼 Скрины", callback_data=f"adm:ub:media:{row.id}"),
                InlineKeyboardButton(text="2️⃣ Текст", callback_data=f"adm:ub:f2:{row.id}"),
                InlineKeyboardButton(text="3️⃣ Текст", callback_data=f"adm:ub:f3:{row.id}"),
            ],
            [
...
async def ub_gift_ask(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    if callback.message:
        await callback.message.answer(
            "Пришли <b>текст подарка</b> с форматированием как в Telegram "
            "(жирный, моноширинный/<code>код</code> = копирование по нажатию, прем-эмодзи).\n"
            "Можно сразу фото + подпись — скрин сохранится.\n"
            "Или HTML: <code>&lt;b&gt;…&lt;/b&gt;</code>, "
            "<code>&lt;code&gt;скопируй&lt;/code&gt;</code>",
            parse_mode="HTML",
        )


async def _save_rich_text(message: Message, aid: int, field_prefix: str) -> None:
    await _merge_premium(message)
    plain, ents = entities_from_aiogram(message)
    kwargs: dict = {}
    if ents:
        kwargs[f"{field_prefix}_text"] = plain
        kwargs[f"{field_prefix}_parse_mode"] = "HTML"
        kwargs[f"{field_prefix}_entities"] = dumps_entities(ents)
    else:
        html = message.html_text or plain or (message.caption or "")
        kwargs[f"{field_prefix}_text"] = html
        kwargs[f"{field_prefix}_parse_mode"] = "HTML"
        kwargs[f"{field_prefix}_entities"] = "[]"
    await update_texts(aid, **kwargs)
...
async def _download_photos(message: Message) -> list[str]:
    ...
    if message.photo:
        ...
        paths.append(f"data/uploads/{name}")
    elif message.document and (message.document.mime_type or "").startswith("image/"):
        suffix = Path(message.document.file_name or "img.jpg").suffix[:8] or ".jpg"
...
async def ub_gift_msg(message: Message, state: FSMContext) -> None:
    ...
    if not aid:
        await state.clear()
        return
    photos = await _download_photos(message)
    if message.photo or message.document or message.text or message.caption:
        if message.text or message.caption or message.caption_entities or message.entities:
            await _save_rich_text(message, aid, "gift")
        if photos:
            async with SessionLocal() as session:
                row = await session.get(UserBotAccount, aid)
                current = []
                if row:
                    try:
                        current = json.loads(row.gift_media or "[]")
                    except json.JSONDecodeError:
                        current = []
                current = list(current) + photos
                await update_texts(aid, gift_media=current[:10])
        await state.clear()
        await message.answer("🎁 Подарок сохранён" + (f" (+{len(photos)} скрин)" if photos else "") + ".")
        card = await _ub_card(aid)
        if card:
            await message.answer(card[0], parse_mode="HTML", reply_markup=card[1])
        return
...
@router.callback_query(F.data.regexp(r"^adm:ub:media:\d+$"))
async def ub_media_ask(callback: CallbackQuery, state: FSMContext) -> None:
    aid = int(callback.data.split(":")[-1])
    await state.update_data(ub_id=aid)
    await state.set_state(AdminSG.wait_ub_gift_media)
    await callback.answer()
    if callback.message:
        await callback.message.answer(
            "Пришли скрины (фото) — добавлю к подарку.\n"
            "Текст <code>очистить</code> — сбросит все скрины.",
            parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup(
                inline_keyboard=[
                    [InlineKeyboardButton(text="✅ Готово", callback_data=f"adm:ub:v:{aid}")]
                ]
            ),
        )


@router.message(AdminSG.wait_ub_gift_media)
async def ub_media_msg(message: Message, state: FSMContext) -> None:
    if (message.text or "").strip() in SKIP:
        return
    data = await state.get_data()
    aid = int(data.get("ub_id") or 0)
    ...
    if raw in {"очистить", "clear", "reset"}:
        await update_texts(aid, gift_media=[])
        await message.answer("Скрины очищены.")
        return
    photos = await _download_photos(message)
    if not photos:
        await message.answer("Пришли фото или «очистить».")
        return
    async with SessionLocal() as session:
        row = await session.get(UserBotAccount, aid)
        current = []
    ...
    current = list(current) + photos
    await update_texts(aid, gift_media=current[:10])
    await message.answer(f"Добавлено {len(photos)}. Всего скринов: {min(len(current), 10)}.")


@router.callback_query(F.data.regexp(r"^adm:ub:f2:\d+$"))
async def ub_f2_ask(callback: CallbackQuery, state: FSMContext) -> None:
    aid = int(callback.data.split(":")[-1])
    await state.update_data(ub_id=aid)
    await state.set_state(AdminSG.wait_ub_followup2)
    await callback.answer()
    if callback.message:
        await callback.message.answer(
            "2️⃣ Текст после ответа на подарок.\n"
            "Пришли сообщение с нужным форматированием "
            "(моноширинный = копирование по нажатию). Пустое «-» — отключить.",
            parse_mode="HTML",
        )


@router.callback_query(F.data.regexp(r"^adm:ub:f3:\d+$"))
async def ub_f3_ask(callback: CallbackQuery, state: FSMContext) -> None:
    aid = int(callback.data.split(":")[-1])
    await state.update_data(ub_id=aid)
    await state.set_state(AdminSG.wait_ub_followup3)
    await callback.answer()
    if callback.message:
        await callback.message.answer(
            "3️⃣ Текст после ответа на второй.\n"
            "Форматирование как в Telegram. «-» — отключить.",
            parse_mode="HTML",
        )


@router.message(AdminSG.wait_ub_followup2)
async def ub_f2_msg(message: Message, state: FSMContext) -> None:
    if (message.text or "").strip() in SKIP:
        return
    data = await state.get_data()
    aid = int(data.get("ub_id") or 0)
    ...
    if (message.text or "").strip() == "-":
        await update_texts(aid, followup2_text="", followup2_entities="[]")
    else:
        await _save_rich_text(message, aid, "followup2")
    await state.clear()
    await message.answer("Текст 2 сохранён.")
    card = await _ub_card(aid)
    if card:
        await message.answer(card[0], parse_mode="HTML", reply_markup=card[1])


@router.message(AdminSG.wait_ub_followup3)
async def ub_f3_msg(message: Message, state: FSMContext) -> None:
    if (message.text or "").strip() in SKIP:
        return
    data = await state.get_data()
    aid = int(data.get("ub_id") or 0)
    if not aid:
        ...
        return
    if (message.text or "").strip() == "-":
        await update_texts(aid, followup3_text="", followup3_entities="[]")
    else:
        await _save_rich_text(message, aid, "followup3")
    await state.clear()
    await message.answer("Текст 3 сохранён.")
    card = await _ub_card(aid)
    if card:
        await message.answer(card[0], parse_mode="HTML", reply_markup=card[1])
...
async def ub_globals(callback: CallbackQuery) -> None:
    ...
    text = (
        ...
        f"API: {api_status_text()}\n"
        f"Макс. ответ (SLA): <b>{g['max_reply_sec']}</b> сек\n"
...
async def ub_set_gap_ask(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    if callback.message:
        await callback.message.answer(
            f"Пришли мин. паузу между отправками в секундах.\nСейчас: <b>{settings.userbot_min_send_gap}</b>",
            parse_mode="HTML",
        )
