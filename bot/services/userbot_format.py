...
_OPEN_RE = re.compile(
    r"<(b|strong|i|em|u|s|strike|del|code|pre|a|tg-emoji|tg-spoiler|spoiler|blockquote)(\s+[^>]*)?>",
    re.IGNORECASE,
)
...
def to_telethon_entities(entities: list[dict] | None):
    ...
    for ent in entities or []:
        ...
        if etype == "bold":
            ...
            # моноширинный / «нажал — скопировал» в мобильном Telegram
            out.append(MessageEntityCode(offset, length))
        elif etype == "pre":
            out.append(MessageEntityPre(offset, length, ent.get("language") or ""))
        elif etype == "spoiler":
            out.append(MessageEntitySpoiler(offset, length))
        elif etype in ("blockquote", "expandable_blockquote"):
            out.append(MessageEntityBlockquote(offset, length, collapsed=etype == "expandable_blockquote"))
