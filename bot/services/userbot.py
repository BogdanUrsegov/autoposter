...
# Антибан: паузы «как человек», но ответ юзеру — не позже MAX_REPLY_LATENCY
MIN_SEND_GAP = 2.0
MIN_SEND_GAP_FLOOR = 0.7
REPLY_DELAY = (1.0, 2.8)
TYPING_SECONDS = (0.8, 2.0)
MAX_REPLY_LATENCY = 60.0  # жёсткий SLA: ответ человеку ≤ 60 сек даже при очереди
FLOOD_WAIT_CAP = 45  # interactive FloodWait не ждём дольше этого
...
def _max_reply_latency() -> float:
    try:
        return max(15.0, float(getattr(settings, "userbot_max_reply_sec", 0) or MAX_REPLY_LATENCY))
    except Exception:
...
async def _send_payload(
    ...
    *,
    ...
    """Отправка с антибаном. deadline (monotonic) — ответ не позже этого момента."""
    client = _clients.get(account_id)
    ...
    try:
        async with _lock(account_id):
            ...
            if remaining <= 0.05:
                # дедлайн почти вышел — шлём сразу без «человечности»
                humanize = False
                min_gap = MIN_SEND_GAP_FLOOR
            else:
                depth = max(1, int(_queue_depth.get(account_id, 1)))
                # при очереди сжимаем паузу, чтобы уложиться в 60с на всех ждущих
                base_gap = _configured_min_gap()
                budget_gap = max(MIN_SEND_GAP_FLOOR, (remaining * 0.55) / depth)
                min_gap = min(base_gap, budget_gap)
                if remaining < 12 or depth >= 4:
                    humanize = False
                elif remaining < 25:
                    humanize = humanize and True

            now = time.monotonic()
            gap = now - _last_send.get(account_id, 0.0)
            if gap < min_gap:
                wait = min_gap - gap
                # не выходим за дедлайн из‑за паузы
                wait = min(wait, max(0.0, deadline - time.monotonic() - 0.2))
                if wait > 0:
            ...
            try:
                ...
                if isinstance(e, FloodWaitError):
                    wait = int(getattr(e, "seconds", 5)) + random.randint(1, 3)
                    left = max(0.0, deadline - time.monotonic() - 0.3)
                    # interactive: не спим дольше SLA / FLOOD_WAIT_CAP
...
async def _handle_incoming(account_id: int, event) -> None:  # noqa: ANN001
    ...
    now = utcnow()
    # SLA: ответ этому человеку максимум за MAX_REPLY_LATENCY секунд
    deadline = time.monotonic() + _max_reply_latency()
    ...
    async with SessionLocal() as session:
        ...
        if lead is None:
            ...
            # короткая «человеческая» пауза, но не съедаем весь SLA
            pause = random.uniform(*REPLY_DELAY)
            pause = min(pause, max(0.0, deadline - time.monotonic() - 8.0))
            if pause > 0:
                await asyncio.sleep(pause)
            ok = await _send_payload(
                account_id, peer_id, hello, parse_mode="HTML", deadline=deadline
            )
            if not ok:
