...
def _start_send_worker(account_id: int) -> asyncio.PriorityQueue:
    """Гарантированно возвращает очередь и живой worker для аккаунта."""
...
async def _fail_pending_jobs(account_id: int) -> None:
    dead: list[tuple[int, int]] = [k for k in _send_jobs if k[0] == account_id]
    for key in dead:
        packed = _send_jobs.pop(key, None)
        if not packed:
            continue
        fut, _kwargs = packed
        if not fut.done():
    ...
    try:
        while _started:
            if account_id not in _clients:
                # клиент отвалился — не зависаем на forever await
                await _fail_pending_jobs(account_id)
                break
            try:
                item = await asyncio.wait_for(q.get(), timeout=45.0)
            except asyncio.TimeoutError:
                if q.empty() and account_id not in _send_jobs:
                    break
                continue
...
async def _send_payload(
    ...
    *,
    ...
    # fallback без очереди, если event loop/воркер недоступны
    try:
        loop = asyncio.get_running_loop()
    except RuntimeError:
        return await _deliver_payload(account_id, **kwargs)

    fut: asyncio.Future = loop.create_future()
    global _job_seq
    _job_seq += 1
    seq = _job_seq
    q = _start_send_worker(account_id)
    _queue_depth[account_id] = int(_queue_depth.get(account_id, 0)) + 1
    _send_jobs[(account_id, seq)] = (fut, kwargs)
    await q.put((int(priority), seq))
    timeout = max(5.0, float(deadline) - time.monotonic() + 2.0)
    try:
        return bool(await asyncio.wait_for(asyncio.shield(fut), timeout=timeout))
    except asyncio.TimeoutError:
        logger.warning("userbot #%s send timeout peer=%s", account_id, peer_id)
        if not fut.done():
            fut.set_result(False)
        return False
    except Exception:
        logger.exception("userbot #%s send queue", account_id)
        if not fut.done():
            fut.set_result(False)
        return False
    finally:
        _queue_depth[account_id] = max(0, int(_queue_depth.get(account_id, 1)) - 1)
        # не трогаем job, если воркер уже забрал; иначе снимаем зависон
