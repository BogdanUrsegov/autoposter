...
async def run_bot() -> None:
    ...
    # handle_signals=False: run.py installs one shared SIGTERM/SIGINT handler for
    # both the bot and the web server — aiogram's own handler would otherwise
    # silently override uvicorn's and the process would hang until SIGKILL.
    await dp.start_polling(
        bot,
        handle_signals=False,
        allowed_updates=list(dict.fromkeys([*dp.resolve_used_update_types(), "channel_post", "message", "callback_query", "chat_join_request"])),
    )
