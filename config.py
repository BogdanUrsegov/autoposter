...
class Settings(BaseSettings):
    ...
    api_id: int = 0
    api_hash: str = ""
    # юзер-бот: макс. задержка ответа человеку (сек) и мин. пауза между сообщениями
    userbot_max_reply_sec: int = 45
    userbot_min_send_gap: float = 0.6
    ...
    @field_validator("userbot_max_reply_sec", mode="before")
    @classmethod
    def _reply_sec(cls, value):
        if value is None or value == "":
            return 45
        try:
            return max(8, min(120, int(value)))
        except (TypeError, ValueError):
            return 45

    @field_validator("userbot_min_send_gap", mode="before")
    @classmethod
    def _send_gap(cls, value):
        if value is None or value == "":
            return 0.6
        try:
            return max(0.2, min(10.0, float(value)))
        except (TypeError, ValueError):
            return 0.6
